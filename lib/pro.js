...
function hasAccess(familyTier, requiredTier) {
  return (TIER_RANK[familyTier] || 0) >= (TIER_RANK[requiredTier] || 0);
}

/** Is this family currently enjoying any paid tier (paid OR trial)? */
function isPro(family) {
  if (!family) return false;
  if (family.proTier === 'lifetime') return true;
  if (family.proTier !== 'none' && family.proExpiresAt && new Date(family.proExpiresAt).getTime() > Date.now()) return true;
...
function hasTierAccess(family, requiredTier) {
  ...
  if (family.trialEndsAt && new Date(family.trialEndsAt).getTime() > Date.now()) return true;
  // Lifetime vault
  if (family.proTier === 'lifetime' && requiredTier === 'vault') return true;
...
function proStatus(family) {
  ...
  let tier = 'none';
  if (family.proTier === 'lifetime') tier = 'lifetime';
  else if (family.proTier !== 'none' && exp > now) tier = family.proTier;
  ...
  return {
    active,
    tier,
    tierName: tier === 'trial' ? 'Vault Trial' : tier === 'lifetime' ? 'Vault Lifetime' : tier !== 'none' ? `${tier.charAt(0).toUpperCase() + tier.slice(1)}` : 'Free',
    expiresAt: family.proTier === 'lifetime' ? null : exp > now ? family.proExpiresAt : null,
...
module.exports = { TRIAL_DAYS, PLANS, TIER_RANK, isPro, hasTierAccess, hasAccess, proStatus, grantPro, requirePro };
